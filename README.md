...
app/
  ...
  page.tsx              Renders <APTSite/>
  globals.css           CSS variables (dark/light), base styles, keyframes
components/
  APTSite.tsx           'use client' — all page markup + mounts the controller
lib/
  aptController.ts      Imperative behavior (Web Audio, GSAP/Lenis, keyboards, toggles)
...
- **Why a controller class?** The original prototype orchestrated everything imperatively against the DOM. Rather than fight that, `APTController` keeps the imperative behavior in one place; `APTSite` renders the markup as JSX and instantiates the controller in a `useEffect`. GSAP, ScrollTrigger, and Lenis are **dynamically imported** inside the effect so nothing touches `window` during server rendering.
